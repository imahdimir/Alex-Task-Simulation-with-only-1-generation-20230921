...
def trio_gwas(y , gts_matrix) :
    ...
    ab = np.zeros((gts_matrix.shape[2] , gts_matrix.shape[1]) ,
                  dtype = np.float_)
...
def compute_pgis(a , new_haps , haps , father_indices , mother_indices , ped) :
    ...
    # pgarray
    pg = pgs.pgarray(pgs_vals ,
                     ped[: , 1] ,
                     sid = np.array(['proband' , 'paternal' , 'maternal']) ,
                     fams = ped[: , 0] ,
                     par_status = np.zeros((pgs_vals.shape[0] , 2)) ,
                     ped = ped)
    return pg

def estimate_direct_effect_pgi(pheno ,
                               ped ,
                               new_haps ,
                               haps ,
                               father_indices ,
    ...
    gts_matrix[: , 0] = proband_genotypes.reshape((proband_genotypes.shape[
                                                       0] * 2 ,
                                                   proband_genotypes.shape[2]) ,
                                                  order = 'C')
    gts_matrix[np.arange(0 , gts_matrix.shape[0] , 2) , 1] = np.sum(
            haps[0][father_indices , 0 , : , :] , axis = 2)
    gts_matrix[np.arange(0 , gts_matrix.shape[0] , 2) , 1] += np.sum(
    ...
    for j in range(gts_matrix.shape[1]) :
        gts_matrix[: , j , :] = gts_matrix[: , j , :] - np.mean(gts_matrix[: ,
                                                                j , :] ,
                                                                axis = 0)
    # Least squares regression
    ab = trio_gwas(y , gts_matrix)
    # Compute PGIs
    pg = compute_pgis(ab[: , 0] ,
                      new_haps ,
                      haps ,
                      father_indices ,
                      mother_indices ,
...
def pgi_analysis(a ,
    ...
    for vratio in [0 , 1 , 10 , 100] :
        ...
        estimate , estimate_cols = pgs.make_and_fit_model(pheno ,
                                                          pg ,
                                                          pg_cols = pg.sid)
        # Estimate parameters
        estimates , ses = pgs.am_adj_2gen(estimate ,
                                          estimate_cols ,
                                          h2f ,
                                          h2f_se ,
                                          rk = rk ,
                                          rk_se = rk_se)
        pgs.write_2gen_adj_ests(estimates ,
                                ses ,
                                outprefix + '_pgi_v' + str(vratio))
    return pg
